CREATE OR REPLACE FORCE VIEW scd.order_item_open_load_time (amp_order_nbr,
                                                            order_item_nbr,
                                                            shipment_id,
                                                            dml_oracle_id,
                                                            dml_tmstmp,
...
AS
   SELECT oio.amp_order_nbr, oio.order_item_nbr, oio.shipment_id,
          ...
          oio.part_key_id, odim.organization_id,
          DECODE (odim.parent_organization_id,
                  odim.company_organization_id, odim.organization_id,
                  odim.parent_organization_id
                 ),
          odim.company_organization_id, accodim.organization_id,
          ppsodim.organization_id, mfrodim.organization_id,
          customer_requested_expedite_dt
...
DROP PUBLIC SYNONYM ORDER_ITEM_OPEN_LOAD_TIME;

CREATE PUBLIC SYNONYM ORDER_ITEM_OPEN_LOAD_TIME FOR SCD.ORDER_ITEM_OPEN_LOAD_TIME;


GRANT SELECT ON SCD.ORDER_ITEM_OPEN_LOAD_TIME TO GBIS_DECISION_SUPPORT_ROLE;

GRANT SELECT ON SCD.ORDER_ITEM_OPEN_LOAD_TIME TO SCD_ADMIN WITH GRANT OPTION;

GRANT SELECT ON SCD.ORDER_ITEM_OPEN_LOAD_TIME TO SCD_PRODUCTION_ROLE;

GRANT SELECT ON SCD.ORDER_ITEM_OPEN_LOAD_TIME TO SCD_SOURCE;

GRANT SELECT ON SCD.ORDER_ITEM_OPEN_LOAD_TIME TO TE_DATAMARTS_WITH_COST;