...
strOrgID = Session("ProdHostOrgID")

GetBrowserVersion strBrowser, strVer, strOS

if Session("ProdHostOrgIDAction") <> "" then
	Session("ProdHostOrgIDAction") = ""	
'	if Len(Trim(Session("ProdHostOrg"))) > 0 then
'		strMode = "Input" 
'	else
		strMode = "Display" 
'	end if
...
Select Case strMode
Case "Return"
	Session("ProdHostOrgIDDescription") = null
	Session("ProdHostOrgIDAction") = "S"
	Response.Redirect strServerUrl & appPath & "search.asp?s=" & vntSessionID
Case "Select"
	Session("ProdHostOrgID") = replace(Request.Form("RowSelected"),", ",",")
	Session("ProdHostOrgIDDescription") = null
	Session("ProdHostOrgIDAction") = "R"
	Response.Redirect strServerUrl & appPath & "search.asp?s=" & vntSessionID
Case "Submit"
	Session("ProdHostOrgID")  = Request.Form("ProdHostOrgID")
	Session("ProdHostOrgIDDescription") = Request.Form("Description")
	strProdHostOrgID = Request.Form("ProdHostOrgID")
	strDescription	 = Request.Form("Description")
	strMode = "Display"		
Case "Display"
	strProdHostOrgID = Session("ProdHostOrgID")
	if Instr(strProdHostOrgID,",") > 0 then
		strProdHostOrgID = ""
	end if
Case Else
	strProdHostOrgID = Session("ProdHostOrgID")
	strDescription   = Session("ProdHostOrgIDDescription")	
	if Instr(strProdHostOrgID,",") > 0 then
		strProdHostOrgID = Left(strProdHostOrgID,Instr(strProdHostOrgID,",")-1)
	end if		
	strMode = "Input"	
end select
...
Function GenHtmlHead()
	...
	response.write "<title>" & Application("AppAbrv") & " - " & "Product Host Org Lookup</title>"
	response.write "</head>"
%>
...
Function GenNavButtons()
	Response.Write "<form name=""ProdHostOrg"" ACTION=""ProdHostOrg.asp"" method=""POST"">"
	...
	Response.Write "    <input TYPE=""submit"" NAME=""SUBMIT"" VALUE=""Select"" onclick=""return(IsProdHostOrgIDSelected(document.forms[0].RowSelected));""></td>" & vbcrlf	
...
'*********************************************************************************************************
Function GetProdHostOrgIDs()
	set objList = server.CreateObject ("DSCD.clsOrgs")
	vntArray = objList.ProdHostOrgIDList(strSQL,vntErrorNumber,vntErrorDesc)
...
Function Criteria()
	response.write "<form method=""POST"" action=""ProdHostOrgID.asp"" id=form1 name=form1>" & vbcrlf
	response.write "<center>" & vbcrlf
	response.write "<table border=""0"" cellspacing=""1"" width=""100%"">" & vbcrlf
	response.write "<td align=""right"" width=""50%"">" & vbCrLf
    response.write "</tr><tr>" & vbCrLf
    response.write "<td align=""right"" width=""50%""><font size=""2""><b>Product Host Org ID:&nbsp;</b></font></td>" & vbCrLf
    response.write "<td align=""left"" width=""50%"">" & vbCrLf
	response.write "<input type=""text"" size=""10"" maxlength=""14"" name=""ProdHostOrgID"" value=""" & strProdHostOrgID & """></td>" & vbCrLf
    response.write "</tr><tr>" & vbCrLf
...
<%
Sub GenIsProdHostOrgIDSelected()
%>
<script language="javascript">
<!--
	function IsProdHostOrgIDSelected(theControl){
