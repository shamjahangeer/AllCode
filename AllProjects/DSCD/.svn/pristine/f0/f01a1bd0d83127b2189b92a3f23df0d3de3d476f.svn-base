...
strOrgID = Session("InvOrgPlantOrgID")

GetBrowserVersion strBrowser, strVer, strOS

if Session("InvOrgPlantAction") <> "" then
	Session("InvOrgPlantAction") = ""	
'	if Len(Trim(Session("InvOrgPlant"))) > 0 then
'		strMode = "Input" 
'	else
		strMode = "Display" 
'	end if
...
Select Case strMode
Case "Return"
	Session("InvOrgPlantDescription") = null
	Session("InvOrgPlantAction") = "S"
	Response.Redirect strServerUrl & appPath & "search.asp?s=" & vntSessionID
Case "Select"
	Session("InvOrgPlant") = replace(Request.Form("RowSelected"),", ",",")
	Session("InvOrgPlantDescription") = null
	Session("InvOrgPlantAction") = "R"
	Response.Redirect strServerUrl & appPath & "search.asp?s=" & vntSessionID
Case "Submit"
	Session("InvOrgPlant")  = Request.Form("InvOrgPlant")
	Session("InvOrgPlantDescription") = Request.Form("Description")
	strInvOrgPlant	= Request.Form("InvOrgPlant")
	strDescription	= Request.Form("Description")
	strMode = "Display"		
Case "Display"
	strInvOrgPlant = Session("InvOrgPlant")
	if Instr(strInvOrgPlant,",") > 0 then
		strInvOrgPlant = ""
	end if
Case Else
	strInvOrgPlant = Session("InvOrgPlant")
	strDescription = Session("InvOrgPlantDescription")	
	if Instr(strInvOrgPlant,",") > 0 then
		strInvOrgPlant = Left(strInvOrgPlant,Instr(strInvOrgPlant,",")-1)
	end if		
	strMode = "Input"	
end select
...
Function GenHtmlHead()
	...
	response.write "<title>" & Application("AppAbrv") & " - " & "Inventory Org/Plant Lookup</title>"
	response.write "</head>"
%>
...
<%
	lsHdr = "Inventory Org/Plant Lookup "
...
Function GenNavButtons()
	Response.Write "<form name=""InvOrgPlant"" ACTION=""InvOrgPlant.asp"" method=""POST"">"
	...
	Response.Write "    <input TYPE=""submit"" NAME=""SUBMIT"" VALUE=""Select"" onclick=""return(IsInvOrgPlantSelected(document.forms[0].RowSelected));""></td>" & vbcrlf	
...
'*********************************************************************************************************
'Inv Org/Plant		vntArray(0)	
'Desc			    vntArray(1)	
'Inv Org or Plant	vntArray(3)	
'*********************************************************************************************************
Function GetInvOrgPlants()
	set objList = server.CreateObject ("DSCD.clsCodeApplXref")
	vntArray = objList.InvOrgPlantList(strOrgID,strInvOrgPlant,strDescription,strExactMatch,strSQL,vntErrorNumber,vntErrorDesc)
	...
	With Response
		.Write "<table border=""1"" cellpadding=""0"" cellspacing=""0"" width=""75%"" align=""center"">"
  			.Write "<tr" & sTableHeadBg & ">"
     			...
     			.Write "<th ALIGN=""center""><font size=""2"" color = White>Inv Org ID/Plant Nbr</th>"
          		.Write "<th ALIGN=""center""><font size=""2"" color = White>Description</th>"
          		.Write "<th ALIGN=""center""><font size=""2"" color = White>Org or Plant</th>"
			.Write "</tr>"
...
Function Criteria()
	response.write "<form method=""POST"" action=""InvOrgPlant.asp"" id=form1 name=form1>" & vbcrlf
	response.write "<center>" & vbcrlf
	response.write "<table border=""0"" cellspacing=""1"" width=""100%"">" & vbcrlf
	response.write "<td align=""right"" width=""50%"">" & vbCrLf
    response.write "</tr><tr>" & vbCrLf
    response.write "<td align=""right"" width=""50%""><font size=""2""><b>Inv Org ID/Plant Number:&nbsp;</b></font></td>" & vbCrLf
    response.write "<td align=""left"" width=""50%"">" & vbCrLf
	response.write "<input type=""text"" size=""10"" maxlength=""14"" name=""InvOrgPlant"" value=""" & strInvOrgPlant & """></td>" & vbCrLf
    response.write "</tr><tr>" & vbCrLf
...
<%
Sub GenIsInvOrgPlantSelected()
%>
<script language="javascript">
<!--
	function IsInvOrgPlantSelected(theControl){
