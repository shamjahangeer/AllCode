CREATE OR REPLACE FORCE VIEW scd.order_item_open_v (amp_order_nbr,
                                                    order_item_nbr,
                                                    shipment_id,
                                                    dml_oracle_id,
                                                    dml_tmstmp,
...
AS
   SELECT amp_order_nbr, order_item_nbr, shipment_id, dml_oracle_id,
          ...
          ROUND (  gfc_extended_true_amp_cost
                 / DECODE (remaining_qty_to_ship,
                           0, NULL,
                           remaining_qty_to_ship
                          ),
                 5
                ),
          part_key_id, storage_location_id, requested_on_dock_dt,
          scheduled_on_dock_dt, sap_profit_center_cde,
          distr_ship_when_avail_ind, header_cust_order_received_dt,
          vendor_key_id, vendor_id, customer_off_credit_hold_date,
          ...
          , cust_pur_ord_line_item_nbr_id, expedite_indicator_cde,
          expedite_status_desc, customer_requested_expedite_dt
     FROM order_item_open
          WITH READ ONLY;


DROP PUBLIC SYNONYM ORDER_ITEM_OPEN_V;

CREATE PUBLIC SYNONYM ORDER_ITEM_OPEN_V FOR SCD.ORDER_ITEM_OPEN_V;


GRANT SELECT ON SCD.ORDER_ITEM_OPEN_V TO GBIS_DECISION_SUPPORT_ROLE;

GRANT SELECT ON SCD.ORDER_ITEM_OPEN_V TO SCD_ADMIN WITH GRANT OPTION;

GRANT SELECT ON SCD.ORDER_ITEM_OPEN_V TO SCD_PRODUCTION_ROLE;

GRANT SELECT ON SCD.ORDER_ITEM_OPEN_V TO SCD_SOURCE;

GRANT SELECT ON SCD.ORDER_ITEM_OPEN_V TO TE_DATAMARTS_WITH_COST;