...
strOrgID = Session("SalesGroupOrgID")
strSalesOffice = Session("SalesGroupSalesOffice")

GetBrowserVersion strBrowser, strVer, strOS

if Session("SalesGroupAction") <> "" then
	Session("SalesGroupAction") = ""	
'	if Len(Trim(Session("SalesGroup"))) > 0 then
'		strMode = "Input" 
'	else
		strMode = "Display" 
'	end if
...
Select Case strMode
Case "Return"
	Session("SalesGroupDescription") = null
	Session("SalesGroupAction") = "S"
	Response.Redirect strServerUrl & appPath & "search.asp?s=" & vntSessionID
Case "Select"
	Session("SalesGroup") = replace(Request.Form("RowSelected"),", ",",")
	Session("SalesGroupDescription") = null
	Session("SalesGroupAction") = "R"
	Response.Redirect strServerUrl & appPath & "search.asp?s=" & vntSessionID
Case "Submit"
	Session("SalesGroup") = Request.Form("SalesGroup")
	Session("SalesGroupDescription") = Request.Form("Description")
	strSalesGroup	= Request.Form("SalesGroup")
	strDescription	= Request.Form("Description")
	strMode = "Display"		
Case "Display"
	strSalesGroup = Session("SalesGroup")
	if Instr(strSalesGroup,",") > 0 then
		strSalesGroup = ""
	end if			
Case Else
	strSalesGroup  = Session("SalesGroup")
	strDescription = Session("SalesGroupDescription")	
	if Instr(strSalesGroup,",") > 0 then
		strSalesGroup = Left(strSalesGroup,Instr(strSalesGroup,",")-1)
	end if			
	strMode = "Input"	
end select
...
Function GenHtmlHead()
	...
	response.write "<title>" & Application("AppAbrv") & " - " & "Sales Group Lookup</title>"
	response.write "</head>"
%>
...
Function GenNavButtons()
	Response.Write "<form name=""SalesGroup"" ACTION=""SalesGroup.asp"" method=""POST"">"
	...
	Response.Write "    <input TYPE=""submit"" NAME=""SUBMIT"" VALUE=""Select"" onclick=""return(IsSalesGroupSelected(document.forms[0].RowSelected));""></td>" & vbcrlf	
...
'*********************************************************************************************************
Function GetSalesGroups()
	set objList = server.CreateObject ("DSCD.clsCodeApplXref")
	vntArray = objList.SalesGroupList(strOrgID,strSalesOffice,strSalesGroup,strDescription,strExactMatch,strSQL,vntErrorNumber,vntErrorDesc)
...
Function Criteria()
	response.write "<form method=""POST"" action=""SalesGroup.asp"" id=form1 name=form1>" & vbcrlf
	response.write "<center>" & vbcrlf
	response.write "<table border=""0"" cellspacing=""1"" width=""100%"">" & vbcrlf
	response.write "<td align=""right"" width=""50%"">" & vbCrLf
    response.write "</tr><tr>" & vbCrLf
    response.write "<td align=""right"" width=""50%""><font size=""2""><b>Sales Group:&nbsp;</b></font></td>" & vbCrLf
    response.write "<td align=""left"" width=""50%"">" & vbCrLf
	response.write "<input type=""text"" size=""4"" maxlength=""14"" name=""SalesGroup"" value=""" & strSalesGroup & """></td>" & vbCrLf
    response.write "</tr><tr>" & vbCrLf
...
<%
Sub GenIsSalesGroupSelected()
%>
<script language="javascript">
<!--
	function IsSalesGroupSelected(theControl){
