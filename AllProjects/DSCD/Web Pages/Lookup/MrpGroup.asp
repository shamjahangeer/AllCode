...
strOrgID = Session("MrpGroupOrgID")

GetBrowserVersion strBrowser, strVer, strOS

if Session("MrpGroupAction") <> "" then
	Session("MrpGroupAction") = ""	
'	if Len(Trim(Session("MrpGroup"))) > 0 then
'		strMode = "Input" 
'	else
		strMode = "Display" 
'	end if
...
Select Case strMode
Case "Return"
	Session("MrpGroupDescription") = null
	Session("MrpGroupAction") = "S"
	Response.Redirect strServerUrl & appPath & "search.asp?s=" & vntSessionID
Case "Select"
	Session("MrpGroup") = replace(Request.Form("RowSelected"),", ",",")
	Session("MrpGroupDescription") = null
	Session("MrpGroupAction") = "R"
	Response.Redirect strServerUrl & appPath & "search.asp?s=" & vntSessionID
Case "Submit"
	Session("MrpGroup")  = Request.Form("MrpGroup")
	Session("MrpGroupDescription") = Request.Form("Description")
	strMrpGroup	    = Request.Form("MrpGroup")
	strDescription	= Request.Form("Description")
	strMode = "Display"		
Case "Display"
	strMrpGroup = Session("MrpGroup")
	if Instr(strMrpGroup,",") > 0 then
		strMrpGroup = ""
	end if		
Case Else
	strMrpGroup    = Session("MrpGroup")
	strDescription = Session("MrpGroupDescription")	
	if Instr(strMrpGroup,",") > 0 then
		strMrpGroup = Left(strMrpGroup,Instr(strMrpGroup,",")-1)
	end if	
	strMode = "Input"	
end select
...
Function GenHtmlHead()
	...
	response.write "<title>" & Application("AppAbrv") & " - " & "MRP Group Lookup</title>"
	response.write "</head>"
%>
...
Function GenNavButtons()
	Response.Write "<form name=""MrpGroup"" ACTION=""MrpGroup.asp"" method=""POST"">"
	...
	Response.Write "    <input TYPE=""submit"" NAME=""SUBMIT"" VALUE=""Select"" onclick=""return(IsMrpGroupSelected(document.forms[0].RowSelected));""></td>" & vbcrlf	
...
'*********************************************************************************************************
Function GetMrpGroups()
	set objList = server.CreateObject ("DSCD.clsCodeApplXref")
	vntArray = objList.MrpGroupList(strOrgID,strMrpGroup,strDescription,strExactMatch,strSQL,vntErrorNumber,vntErrorDesc)
	...
	With Response
		.Write "<table border=""1"" cellpadding=""0"" cellspacing=""0"" width=""75%"" align=""center"">"
  			.Write "<tr" & sTableHeadBg & ">"
     			...
     			.Write "<th ALIGN=""center""><font size=""2"" color = White>MRP Group</th>"
...
Function Criteria()
	response.write "<form method=""POST"" action=""MrpGroup.asp"" id=form1 name=form1>" & vbcrlf
	response.write "<center>" & vbcrlf
	response.write "<table border=""0"" cellspacing=""1"" width=""100%"">" & vbcrlf
	response.write "<td align=""right"" width=""50%"">" & vbCrLf
    response.write "</tr><tr>" & vbCrLf
    response.write "<td align=""right"" width=""50%""><font size=""2""><b>MRP Group:&nbsp;</b></font></td>" & vbCrLf
    response.write "<td align=""left"" width=""50%"">" & vbCrLf
	response.write "<input type=""text"" size=""15"" maxlength=""20"" name=""MrpGroup"" value=""" & strMrpGroup & """></td>" & vbCrLf
    response.write "</tr><tr>" & vbCrLf
...
<%
Sub GenIsMrpGroupSelected()
%>
<script language="javascript">
<!--
	function IsMrpGroupSelected(theControl){
