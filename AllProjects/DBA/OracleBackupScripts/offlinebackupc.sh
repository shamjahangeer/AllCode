#!/usr/bin/sh
#
# Offline Complete backup of the database
#
#
# MODIFIED
...
{
   echo "" >&2
   echo "Usage:  offlinebackupc.sh <ORACLE_HOME> <ORACLE_SID> <BACKUP_DIR>" >&2
#   echo "Usage:  offlinebackupc.sh $ORACLE_HOME $ORACLE_SID /opt/db03/backup/offline" >&2
   echo "" >&2
...
-- copy data files
  SELECT '!cp -p ' || RTRIM(name) || ' $BACKUP_DIR'
    FROM v\$datafile;
-- copy temp data files
  SELECT '!cp -p ' || RTRIM(name) || ' $BACKUP_DIR'
    FROM v\$tempfile;
-- copy redo log files
  SELECT '!cp -p ' || RTRIM(member) || ' $BACKUP_DIR'
    FROM v\$logfile;
-- copy control files
  SELECT '!cp -p ' || RTRIM(name) || ' $BACKUP_DIR'
    FROM v\$controlfile;
...
# backup server parameter file & password file

cp -p ${ORACLE_HOME}/dbs/spfile${ORACLE_SID}.ora ${BACKUP_DIR}
cp -p ${ORACLE_HOME}/dbs/orapw${ORACLE_SID} ${BACKUP_DIR}
