...
source /script/oracle/env/Oracle_Env_Files.sh
...
find ${DUMP_DIR1} -name \*.dmp -mtime +4 -exec rm {} \;
find ${DUMP_DIR} -name \*.log -mtime +4 -exec rm {} \;
...
exp userid=system/manager full=y file=${DUMP_DIR1}/full_${ORACLE_SID}_$THISDATE.dmp log=${LOG_DIR}/full_${ORACLE_SID}_$THISDATE.log STATISTICS=NONE
